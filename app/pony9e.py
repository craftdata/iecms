...
class Document(db.Entity):
    ...
    lock_date = Optional(datetime, sql_type='abstime', sql_default='infinity')  # Table locked
...
class CaseCategory(db.Entity):
    id = PrimaryKey(int, auto=True)
    court_cases = Set(CourtCase)
    casecategories = Set('CaseCategory', reverse='subcategory')
    subcategory = Optional('CaseCategory', reverse='casecategories')
    case_category_checklists = Set('CaseCategory_CaseChecklist')
...
class CaseCategory_CaseChecklist(db.Entity):
    casechecklists = Required(CaseChecklist)
    casecategories = Required(CaseCategory)
    PrimaryKey(casechecklists, casecategories)
