...
from sqlalchemy import (
    ...
    Text, Interval, Enum, BigInteger, Boolean, Column, Date, DateTime, ForeignKey, ForeignKeyConstraint,
    Index, Integer, LargeBinary, Numeric, String, Table, Text, Time, text
)
...
from sqlalchemy_fsm import transition
...
person_fieldset = [
    ...
    ('Identity',
     {'fields': ['firstname', 'surname', 'othernames', 'gender1', 'dob', 'marital_status'], 'expanded': True})
]

person_show_fieldset = [
    ('Picture', {'fields': ['photo_img', 'photo']}),
    ('Identity',
     {'fields': ['firstname', 'surname', 'othernames', 'gender1', 'age', 'dob', 'marital_status'], 'expanded': True})
]
...
employment_fieldset = [
    ...
    ('Employment Dates',
     {'fields': ['employ_role', 'employ_date', 'termination_date', 'supervisor', 'supervisor_contact'],
      'expanded': True}),
]
...
casey_fieldset = [
    ('Initiation',
     {'fields': ['born_digital', 'policestation', 'polofficer', 'ob_number', 'report_date', 'complaint',
                 'casecategory'], 'expanded': True}),
    ('Evaluation',
     {'fields': ['natureofsuit', 'polofficer1', 'plaintiff', 'defendant', 'witness', 'priority',
                 'evaluation_conclusion', 'should_investigate_further'], 'expanded': False}),
    ('Investigation',
     {'fields': ['investigation_assigment_date', 'polofficer2', 'investigation_assignment_note', 'investigation_plan',
                 'investigation_summary', 'investigation_review', 'investigation_complete'], 'expanded': False}),
    ('DPP Advice',
     {'fields': ['dpp_advice_requested', 'dpp_advice_request_date', 'dpp_advice', 'dpp_advice_date', 'send_to_trial',
                 'charge_date'], 'expanded': False}),
    ('Prosecution',
     {'fields': ['causeofaction', 'docketnumber', 'case_name', 'charge_sheet', 'prosecutor', 'prosecution_notes',
                 'defense_notes'], 'expanded': False}),
    ('Judgement',
     {'fields': ['judgement_date', 'judgement', 'case_closed', 'close_date'], 'expanded': False}),
    ('Sentencing',
     {'fields': ['fine_amount', 'sentence_length_years', 'sentence_start_date', 'sentence_end_date', 'case_appealed',
                 'appeal_date', 'appeal_granted'], 'expanded': False}),
    ('Tags',
     {'fields': ['tags']})
]
...
doc_edit_fieldset = [
    ...
    ('Geek Stuff',
     {'fields': ['mime_type', 'file_size_bytes', 'char_count', 'doc_type', 'char_count', 'word_count', 'lines',
                 'paragraphs', 'page_count'], 'expanded': False}),
]
...
investigation_fieldset = [
    ('Activity',
     {'fields': ['investigator', 'case1', 'actiondate', 'action', 'activity_description', 'status'], 'expanded': True}),
