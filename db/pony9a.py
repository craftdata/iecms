...
class Page(db.Entity):
    ...
    image_ext = Optional(str, nullable=True)
...
class Payment(db.Entity):
    ...
    date_paid = Optional(datetime, default=lambda: datetime.now())
    phone_number = Optional(str, 20, nullable=True)
    validated = Optional(bool)
    payment_description = Optional(str, nullable=True)
...
class Sys_View(db.Entity):
    ...
    sys_wkflow_views = Set('Sys_wkflow_view')


class Sys_user_extra(db.Entity):
    ...
    sys_wkflow_cat = Required('Sys_wkflow_cat')


class Sys_wkflow(db.Entity):
    id = PrimaryKey(int, auto=True)
...
class Sys_wkflow_cat(db.Entity):
    ...
    sys_wkflows = Set(Sys_wkflow)
    sys_user_extras = Set(Sys_user_extra)


class Sys_wkflow_view(db.Entity):
    sys__view__lists = Required(Sys_View)
    sys_wkflows = Required(Sys_wkflow)
